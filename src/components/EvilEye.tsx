    ...
    useEffect(() => {
        ...
        program = new Program(gl, {
            ...
            uniforms: {
                ...
                uFlameSpeed: { value: 1.0 }, // Set to 1.0 since dynamic speed is accumulated directly into uTime
        ...
        let lastTime = 0;
        let flameTime = 0;
        let currentCenterFactor = 0; // Starts at 0, resulting in base color (#F97316) and speed 1 when cursor is off screen
        ...
        function update(time: number) {
            ...
            // Calculate pupil position in normalized space [-1, 1]
            const aspect = gl.canvas.height > 0 ? gl.canvas.width / gl.canvas.height : 1;
            const pupil_cx = mouse.x * pupilFollow * 0.12 * scale / aspect;
            const pupil_cy = mouse.y * pupilFollow * 0.12 * scale;

            // Distance of mouse from pupil
            const dist = Math.hypot(mouse.x - pupil_cx, mouse.y - pupil_cy);

            // Target center factor: if mouse is on screen, compute based on distance. If off screen, target is 0.
            const maxDist = 0.8;
            const targetCenterFactor = isMouseOnScreen ? Math.max(0, Math.min(1, 1 - dist / maxDist)) : 0;

            // Smoothly interpolate currentCenterFactor towards targetCenterFactor
            currentCenterFactor += (targetCenterFactor - currentCenterFactor) * 0.05;

            // Speed goes from flameSpeed (outer, default 1) to 3 (inner) as we get to the center
            const currentSpeed = flameSpeed + (3 - flameSpeed) * currentCenterFactor;

            // Accumulate time using the dynamic speed to prevent discontinuous texture jumps
            flameTime += (delta * 0.001) * currentSpeed;

            // Color changes from eyeColor (outer, e.g. #F97316) to #EF4444 (inner) as we get to the center
            const cOuter = hexToVec3(eyeColor);
            const cInner = hexToVec3('#EF4444');
            const r = cOuter[0] + (cInner[0] - cOuter[0]) * currentCenterFactor;
            const g = cOuter[1] + (cInner[1] - cOuter[1]) * currentCenterFactor;
            const b = cOuter[2] + (cInner[2] - cOuter[2]) * currentCenterFactor;

            program.uniforms.uMouse.value = [mouse.x, mouse.y];
            program.uniforms.uTime.value = flameTime;
            program.uniforms.uEyeColor.value = [r, g, b];
